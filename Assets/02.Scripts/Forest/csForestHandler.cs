...
public class csForestHandler : MonoBehaviour
{
    ...
    private void Check()
    {
        ...
        if (gameObject.transform.name.Equals("Bear"))
        {
            csSoundManager.instance.PlayBearHitSound();
        }
        else if (gameObject.transform.name.Equals("Wolf") || gameObject.transform.name.Equals("Fox"))
        {
            csSoundManager.instance.PlayWolfHitSound();
        }
        else if (gameObject.transform.name.Equals("Bird"))
        {
            csForestManager.instance.birdCnt -= 1;
            csSoundManager.instance.PlayBirdHitSound();
        }

        GameObject obj = csPooledForestEffect.instance.GetPooledObject_ForestEffect(gameObject.transform);
        Transform objPosi = obj.transform;
        objPosi.position = new Vector3(objPosi.position.x, objPosi.position.y + 3, objPosi.position.z);
        obj.SetActive(true);

        csPooledAnimal.instance.poolObjs_Animal.Remove(gameObject);
        csPooledAnimal.instance.poolObjs_Animal.Add(gameObject);
